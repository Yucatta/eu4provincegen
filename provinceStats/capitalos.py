            ...
            for asd in f:
                ...
                for i,word in enumerate(words_in_line):
                    if(word == "capital"):
                        ...
                        capitalos.append([filename.split()[0],clean])
                    else:
...
with open("provinces3.csv",mode="r",newline="") as f:
    reader = csv.reader(f)
    for row in reader:
        provinces2.append(row)

for row in capitalos:
    provinces2[int(row[0])-1][1] = row[1]

with open("provinces4.csv",mode="w",newline="")as f:
    writer = csv.writer(f)
